...
function ChatPageClient() {
  ...
  // Listen for window resize to handle mobile states dynamically
  useEffect(() => {
  ...
  return (
    <ToastProvider>
      <ErrorBoundary>
        <div style={{ height: '100dvh' }} className="flex overflow-hidden bg-[#0F1113]">
          {/* Sidebar overlay for mobile/tablet */}
          {sidebarOpen && windowWidth !== null && windowWidth < 1024 && (
            <div
              className="fixed inset-0 bg-[#0F1113]/50 z-40 lg:hidden"
          ...
          <div className="flex flex-row w-full h-full">
            <div
              ...
              } fixed lg:relative inset-y-0 left-0 lg:translate-x-0 lg:p-3 lg:py-6 lg:pl-6 border-b border-[#F4EFE6]/10 h-full`}
              style={{
                width: sidebarOpen
            ...
            <div 
              className="flex-1 flex flex-col overflow-hidden border-b border-[#F4EFE6]/10"
              onClick={() => {
                if (sidebarOpen && windowWidth !== null && windowWidth < 1024) {
                  setSidebarOpen(false);
                }
              }}
            >
              ...
              >
                <span
                  className="text-2xl sm:text-3xl font-extrabold text-[#D4A24F] tracking-wide bg-[#14171B]/95 px-5 py-2 rounded-2xl shadow-xl flex items-center justify-center border border-[#D4A24F]/25 drop-shadow-lg pointer-events-auto"
                  style={{ minHeight: '2.75rem', letterSpacing: '0.08em' }}
                >
                  <div className="relative w-8 h-8 sm:w-9 sm:h-9 rounded-full border border-[#D4A24F]/35 mr-2.5 overflow-hidden flex-shrink-0 flex items-center justify-center bg-[#14171B] isolate">
                    <img src="/hero_dragon.jpg" alt="Dragon GPT" className="w-full h-full object-cover scale-[1.15] object-center relative z-10" />
                  </div>
                  Dragon <span className="ml-1.5 font-light text-[#F4EFE6]/80">GPT</span>
                </span>
              </div>
